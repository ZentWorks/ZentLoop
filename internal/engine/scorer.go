...
var pathSignals = []struct {
	...
	{"/fetch", 32, "ssrf-discovery", "api"}, {"/$(pwd)/", 48, "file-read", "devops"}, {"/var/www/html/.env", 48, "file-read", "devops"}, {"/proxy", 32, "ssrf-discovery", "api"}, {"/@fs/", 52, "file-read", "devops"}, {"/proc/self/environ", 52, "file-read", "devops"}, {"terraform.tfstate", 48, "credential-discovery", "cloud"}, {"terraform.tfvars", 46, "credential-discovery", "cloud"}, {".azure/", 46, "credential-discovery", "cloud"}, {".npmrc", 42, "credential-discovery", "devops"}, {".netrc", 44, "credential-discovery", "devops"}, {"rclone.conf", 44, "credential-discovery", "cloud"}, {"/latest/meta-data/", 52, "cloud-metadata-discovery", "cloud"}, {"/db.sql", 40, "backup-discovery", "database"}, {"/dump.sql", 40, "backup-discovery", "database"},
	...
	{"/wp-json/", 30, "cms-enumeration", "wordpress"}, {"/wp/v2/", 30, "cms-enumeration", "wordpress"}, {"credentials.yml.enc", 46, "credential-discovery", "rails"}, {"secret_token.rb", 44, "credential-discovery", "rails"}, {"/.svn/", 36, "source-discovery", "devops"}, {"/.hg/", 36, "source-discovery", "devops"}, {"aws.ini", 38, "credential-discovery", "cloud"}, {"s3.yml", 38, "credential-discovery", "cloud"}, {"s3.yaml", 38, "credential-discovery", "cloud"}, {".sql.gz", 42, "backup-discovery", "database"},
...
var measurementUA = []string{"censysinspect", "shodan", "internet-measurement"}
...
func (s *Scorer) AssessAt(r *http.Request, ss *model.Session, bodySample string, now time.Time) Assessment {
	...
	for _, x := range measurementUA {
		if strings.Contains(ua, x) {
			autoDelta += 75
			cat = "internet-measurement"
			break
		}
	}
